...
class VideoExplainerView: UIView {
  ...
  init() {
    ...
    restartButton.addTarget(
      self, action: #selector(restartButtonTapped(sender:)), for: .touchUpInside)
  ...
  override func layoutSubviews() {
    ...
    restartButton.frame = CGRect(
      x: bounds.width - restartButtonSize.width - restartButtonEdgeSpacing,
      y: bounds.height - restartButtonSize.height - restartButtonEdgeSpacing,
      width: restartButtonSize.width, height: restartButtonSize.height)
  }
  ...
  private func setUpVideo() {
    ...
    let newAsset = AVAsset(url: Bundle.main.url(forResource: "helpMovie", withExtension: "mp4")!)
